...
async def test_controller_create_should_return_success(cliente, products_url):
...
async def test_controller_get_should_return_not_found(client, products_url):
    response = await client.get(f"{products_url}4fd7cd35-a3a0-4c1df-a78d-d24aa81e7dca")

    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {'detail': "Product not found with filter: 4fd7cd35-a3a0-4c1df-a78d-d24aa81e7dca"}

@pytest.marj.usefixtures("products_inserted")
...
async def test_controller_delete_should_return_not_found(client, products_url):
    response = await client.delete(f"{products_url}4fd7cd35-a3a0-4c1df-a78d-d24aa81e7dca")
